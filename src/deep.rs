use std::fmt;
use serde::{de::Visitor, Deserialize, Serialize, Deserializer, Serializer, de::SeqAccess};
...
fn write_file(){
    ...
    let order_book_origin = BinanceSpotOrderBookSnapshot{
        time_stamp:20,
        last_update_id: 20,
        bids: vec![DepthRow{price: 1.0, amount: 3.0}],
        asks: vec![DepthRow{price: 1.0, amount: 3.0}],
    };
